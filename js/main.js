...
(() => {
    ...
    */
    ...
    function changeImageset() {
        ...
        let clickThumb = this; // this is the element (thumnail) we clicked on 
        //debugger; 
        // this wil pause code execution on this line, like pushing pause on Netflix / Amazon Prime
        dropZones.forEach((zone) => {
            while (zone.firstChild) {
                zone.removeChild(zone.firstChild);
            }
        });
        // upadate the draggable piece's src attribute one at the time  
        pzlPieces.forEach((piece, index) => {
            piece.src = `images/${imageNames[index] + clickThumb.dataset.bgref}.jpg`;
        });
        reset();
    }
    ...
    function allowDrop(event) {
        ...
        // if this.childElementCount is greater than 0, dont' execute anything else, just  
    ...
    function reset() {
        ...
        var pzlPiecesReset = document.getElementsByClassName('puzzle-pieces')[0]
        pzlPieces.forEach((piece) => {
            pzlPiecesReset.appendChild(piece);
        })
    }
